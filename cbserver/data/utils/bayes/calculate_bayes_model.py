...
def determine_model(user_id):
	user = user_collection.find_ond({"user_id": user_id})
	if user:
...
def calculate_rankings(user_id):
	...
	rec_attributes = pickle.load(open(DUMP_RECOMMENDATIONS_ATTR,"rb"))
	test_attributes = rec_attributes.values()
	sparse_test = sparse.csr_matrix(np.array(test_attributes))

	predicted_classes = mnb.predict(sparse_test)
	
	# we have the predicted classes for all of the recommendations 
